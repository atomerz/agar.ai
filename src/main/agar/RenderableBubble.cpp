...
RenderableBubble::RenderableBubble(IRenderEngine* renderEngine, float mass, float x, float y, std::unique_ptr<Genome> genome)
  : Bubble(mass, x, y, std::move(genome)), fovColor(0.3f, 0.3f, 0.3f) {
  init(renderEngine);
  color = makeGeneticColor();
}

RenderableBubble::RenderableBubble(IRenderEngine* renderEngine, float mass, Coord2d c, std::unique_ptr<Genome> genome)
	: Bubble(mass, c, std::move(genome)), fovColor(0.3f, 0.3f, 0.3f) {
  init(renderEngine);
  color = makeGeneticColor();
}
...
void RenderableBubble::reset(float mass, float x, float y, std::unique_ptr<Genome> genome) {
	Bubble::reset(mass, x, y, std::move(genome));
	color = makeGeneticColor();
}

void RenderableBubble::reset(float mass, Coord2d c, std::unique_ptr<Genome> genome) {
...
Color RenderableBubble::makeGeneticColor() {
  if (getBrain() == nullptr) return Color::randomize();

  auto& genome = getBrain()->getGenome().getGenes();
